...
实现 GitLab 平台的 MCP 服务器。
"""
from typing import List, Dict, Any, Optional
from datetime import datetime, timedelta
from .base import MCPServerBase
from ..core.fetchers import GitLabFetcher
...
class GitLabMCPServer(MCPServerBase):
    ...
    def __init__(self, config: Any):
        ...
        Args:
            config: 用户配置对象，必须包含 gitlab_url 和 gitlab_token
        """
        super().__init__(config)

        # 验证配置
        if not hasattr(config, 'gitlab_url') or not config.gitlab_url:
            raise ValueError("gitlab_url is required in config")

        if not hasattr(config, 'gitlab_token') or not config.gitlab_token:
            raise ValueError("gitlab_token is required in config")

        # 初始化 GitLab 获取器
        self.fetcher = GitLabFetcher(
            url=config.gitlab_url,
            token=config.gitlab_token
        )

        logger.info(f"GitLab MCP Server initialized for user {self.user_id}")
        ...
        return [
            {
                ...
                "description": "Get Git commit history from GitLab projects",
                "inputSchema": {
                    "type": "object",
                    "properties": {
                        "since_date": {
                        ...
                        "project_id": {
                            "type": "string",
                            "description": "Specific project ID to filter commits"
                        }
                    }
                }
            },
            {
                ...
                "description": "Get list of GitLab projects accessible to the user",
                "inputSchema": {
                    "type": "object",
                    "properties": {}
                }
            ...
            {
                "name": "search_gitlab_commits",
                "description": "Search GitLab commits by message content",
                "inputSchema": {
                    "type": "object",
                    "properties": {
                        "query": {
    ...
    async def call_tool(self, name: str, arguments: Dict[str, Any]) -> Any:
        """调用工具"""
        try:
            if name == "get_gitlab_commits":
                return await self._get_commits_tool(arguments)
            elif name == "get_gitlab_projects":
                return await self._get_projects_tool()
            elif name == "get_gitlab_project":
                return await self._get_project_tool(arguments)
            elif name == "search_gitlab_commits":
    ...
    async def get_commits(
        ...
        logger.info(f"Fetching GitLab commits for user {self.user_id}")
        commits = await self.fetcher.get_commits(since_date, until_date, branch, project_id)

        logger.info(f"Retrieved {len(commits)} commits from GitLab")
        return commits

    async def get_projects(self) -> List[GitProject]:
        """获取项目列表"""
        logger.info(f"Fetching GitLab projects for user {self.user_id}")
    ...
    async def _get_commits_tool(self, arguments: Dict[str, Any]) -> Dict[str, Any]:
        ...
        commits = await self.get_commits(since_date, until_date, branch, project_id)
    ...
    async def _get_projects_tool(self) -> Dict[str, Any]:
        """获取项目列表工具实现"""
    ...
    async def _get_project_tool(self, arguments: Dict[str, Any]) -> Dict[str, Any]:
        """获取单个项目工具实现"""
        project_id = arguments.get("project_id")
        ...
        project = await self.fetcher.get_project(project_id)
